...
function renderAll(){renderStats();renderFilters();renderRows();renderStaff();renderSubmissionStats();renderSubmissionFilters();renderSubmissions();lucide.createIcons()}
...
async function saveSubmissionEdits(silent=false){if(!reviewingSubmissionId)return null;const payload=reviewPayload();if(!payload.full_name||!payload.batch){reviewStatus('กรุณาระบุชื่อ-นามสกุลและรุ่น',false);return null}const {data,error}=await db.from('alumni_submissions').update(payload).eq('id',reviewingSubmissionId).select('*').single();if(error){reviewStatus('บันทึกไม่สำเร็จ: '+error.message,false);return null}const idx=submissions.findIndex(x=>x.id===reviewingSubmissionId);if(idx>=0)submissions[idx]=data;if(!silent){reviewStatus('บันทึกการแก้ไขแล้ว');renderSubmissionStats();renderSubmissionFilters();renderSubmissions()}return data}
...
$('#review-approve-btn').addEventListener('click',async()=>{
  ...
  try{
    ...
    const photo=await promotePrivatePhoto(x);const alumniPayload={full_name:x.full_name,photo_url:photo,batch:x.batch,graduation_year:x.graduation_year||'',graduation_level:x.graduation_level||'',current_position:x.current_position||'',occupation:x.occupation||'',organization:x.organization||'',education:x.education||'',phone:x.phone||'',email:x.email||'',facebook_url:x.facebook_url||'',bio:x.bio||'',show_contact:!!x.show_contact,featured:!!x.featured,published:true,sort_order:0};
...
// ---------------- Navigation ----------------
const viewMeta={directory:['รายชื่อศิษย์เก่า','เพิ่ม แก้ไข ลบ และจัดการการเผยแพร่'],submissions:['แบบฟอร์มที่ส่งมา','ตรวจสอบ แก้ไข อนุมัติ หรือไม่อนุมัติก่อนเผยแพร่'], 'staff-alumni':['บุคลากรที่เป็นศิษย์เก่า','ข้อมูลนี้เชื่อมจาก Website Manager อัตโนมัติ']};
